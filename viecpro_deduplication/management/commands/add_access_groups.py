...
import json
from viecpro_deduplication.models import PersonProxy, Group, Suggestions, StatusButtonProxyType, StatusButtonGroupType, StatusButtonGroup, StatusButtonProxyType, StatusButtonProxy
from apis_core.apis_entities.models import Person
from apis_core.apis_metainfo.models import Collection
import logging
from django.contrib.auth.models import User
from viecpro_deduplication.logger import init_logger
...
class Command(BaseCommand):
    ...
    def create_new_groups(self, per_set):
        ...
        df_per = pd.DataFrame(selection.values("id", "name", "first_name", "gender", "start_date",
                              "end_date", "start_date_written", "end_date_written")).set_index("id")
        per = df_per[["name", "first_name", "gender", "start_date",
                      "end_date", "start_date_written", "end_date_written"]]
        ...
        for idx, (d, group) in enumerate(per.groupby(["name", "first_name", "gender", "sd", "ed"])):
            ...
            if len(group) > 1:
                ...
                for ix in index:
                    gm, c = PersonProxy.objects.get_or_create(
                        person=Person.objects.get(id=ix))
                    gm.status = "candidate"
                    gm.save()
                    g.members.add(gm)
                g._gender = d[2]
    ...
    def create_ampel_buttons(self):
        ...
        types = [("Checked Members", "CM"), ("Checked Suggestions", "CSu"),
                 ("Checked Singles", "CSi"), ("Ready To Merge", "RM")]
        for el in types:
            btn, c = StatusButtonGroupType.objects.get_or_create(
                name=el[0], short=el[1])
    ...
    def calculate_names(self):
        ...
        [p_n[tuple(p.allnames)].append(p.id)
         for p in PersonProxy.objects.all()]
        [p_f[tuple(p.allfirst_names)].append(p.id)
         for p in PersonProxy.objects.all()]
    ...
    def handle(self, *args, **options):
        nopps = [p for p in Person.objects.all(
        ) if not hasattr(p, "personproxy")]
