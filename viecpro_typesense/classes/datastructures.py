...
class O:
    ...
    def to_dict(self):
        return {k: v for k, v in asdict(self).items()} # todo: remove unpacking, this is redundant
        ...
        if isinstance(data, dict):
            for k,v in data.items():
                if hasattr(self, k):
                    ...
                    raise Exception(f"Tried to merge your O-object with attributes not allowd in O.")
